...
public class TrackerUI : MonoBehaviour
{
    ...
    ////////////////////////////////////////////////////////////////
    /// 
    private List<string> _numberTestList = new List<string> { "1", "2", "3" };
	...
	void OnGUI ()
	{
		...
		if (_menuAction == MenuAction.NetworkSettings)
        {
            ...
            if (GUI.Button(new Rect(left + 80, top, 25, 25), NextTexture))
            {
                _numberTest++;
                if (_numberTest < 0 || _numberTest >= 3) _numberTest = 0;
            }

            if (GUI.Button(new Rect(left + 120, top, 90, 25), "Reset Testes"))
            {
                _numberTest = 0;
            }
            top += 30;
          
            if (GUI.Button(new Rect(left, top, 80, 25), "End Test"))
            {
            ...
            else
            {
                ...
                if (_localHandleVirtualWorld.CanShowIndicators || _localHandleVirtualWorld.IsCentroSetUp)
                {
                    ...
                    if (_localHandleVirtualWorld.CanShowIndicators)
                    {
                        ShowIndicator = GUI.Toggle(new Rect(left + moreLeft, top, 100, 25), ShowIndicator,
                            "Show Indicators");
                        _localHandleVirtualWorld.ShowIndicator = ShowIndicator;
                    }
                }
...
var f = 1.0f;
            ...
            GUI.Slider(new Rect(left, top, 100, 25), f, 10.0f, 0.0f, 3.0f, _titleStyle,_titleStyle, true, 0);
             top += 25;
